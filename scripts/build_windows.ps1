...
if ([string]::IsNullOrWhiteSpace($env:RENPY_SDK_PATH)) {
    throw "RENPY_SDK_PATH is not set. Point it at a local Ren'Py SDK root before running build_windows.ps1."
}
...
& $PythonExe $RenpyPy launcher distribute $ResolvedProjectPath --package $Package --no-update
exit $LASTEXITCODE
